...
function DQ_rules(d, p, n_s)
    ...
    run(`'/Applications/MATLAB_R2024a.app/bin/maca64/MATLAB' -nodisplay -nosplash -nodesktop -r "addpath(genpath('.')); [XW,deltamain]=generator($d,$p,$n_s); save('DQ_rules.mat', 'XW'); exit;"`)

    # Load the output from MATLAB
    mat = matread("DQ_rules.mat")
    XW = mat["XW"]
...
function DQ_integrate(f, d, p, n_s)
    ...
    nodes, weights = DQ_rules(d, p, n_s)
